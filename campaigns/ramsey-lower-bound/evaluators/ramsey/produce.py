"""Trusted producer wrapper for one Ramsey certificate."""
...
class _BlockedSocket:
    def __init__(self, *args: object, **kwargs: object) -> None:
        del args, kwargs
        raise RuntimeError("network disabled in Ramsey certificate producer")
...
def _snapshot_value(raw: object, path: str) -> object:
    """Read one candidate value once and return only plain JSON-compatible values."""
...
def snapshot_certificate(raw: object) -> dict[str, object]:
    """Snapshot a candidate return without trusting repeated container reads."""
...
def canonical_bytes(snapshot: dict[str, object]) -> bytes:
    """Serialize one plain snapshot in the only accepted wire format."""
...
def _load_candidate(candidate_dir: Path) -> ModuleType:
    ...
    name = f"_ramsey_candidate_{os.getpid()}"
...
def _call_construct(module: ModuleType, s: int, n_cap: int, deadline: float) -> object:
    ...
    try:
        signature.bind(s, n_cap, deadline)
    except TypeError:
        try:
            signature.bind(s, n_cap, deadline=deadline)
        except TypeError:
            signature.bind(s, n_cap)
            return construct(s, n_cap)
        return construct(s, n_cap, deadline=deadline)
    return construct(s, n_cap, deadline)
...
def _run(argv: list[str]) -> int:
    if len(argv) != 6:
        raise ValueError(
            "usage: produce.py CANDIDATE_DIR OUTPUT_PATH S N_CAP SEARCH_BUDGET_S"
        )
    candidate_dir = Path(argv[1]).resolve()
    output_path = Path(argv[2]).resolve()
    s = int(argv[3])
    n_cap = int(argv[4])
    search_budget_s = float(argv[5])
    ...
    snapshot = snapshot_certificate(_call_construct(module, s, n_cap, deadline))
    _write_atomic(output_path, canonical_bytes(snapshot))
    return 0


def main(argv: list[str] | None = None) -> NoReturn:
    """Run once, with stdout permanently disconnected from any verdict channel."""
    ...
    except BaseException as exc:  # noqa: BLE001 - producer must leave a useful stderr tail
